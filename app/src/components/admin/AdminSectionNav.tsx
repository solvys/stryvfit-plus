...
import { CalendarClock, Dumbbell, Salad, UsersRound } from 'lucide-react';
import { motion } from 'framer-motion';

type AdminSection = 'appointments' | 'workouts' | 'meals' | 'clients';
...
export function AdminSectionNav({ active, onAppointments, onMeals, onClients }: AdminSectionNavProps) {
  return (
    <nav
      ...
      className="grid min-h-14 min-w-full grid-cols-4 gap-1 rounded-md border border-[#dedbd4] bg-white p-1 shadow-sm sm:min-w-[560px]"
    >
      {sections.map((section) => {
        ...
        const className = `relative inline-flex min-h-12 items-center justify-center rounded-md bg-transparent px-3 font-caption text-[10px] uppercase tracking-[0.14em] transition-colors ${
